...
def do(sim,LFMR):
    ...
    if (LFMR):
        plt.text( 0.75, 0.15, r"${\rm Local~ FMR}$", transform=plt.gca().transAxes, ha='center' )
    else:
        plt.text( 0.75, 0.15, r"${\rm Global~ FMR}$", transform=plt.gca().transAxes, ha='center' )
    plt.text( 0.75, 0.1, r"${\rm All~}z$", transform=plt.gca().transAxes )
    plt.text( 0.05, 0.9, "%s" %WHICH_SIM_TEX[sim], transform=plt.gca().transAxes )
